...
class DataProcessing:
    ...
    def filter_users(self, min_ratings=400):
        try:
            ...
            logger.error("Error while filtering users")
            raise CustomException(f"Failed to filter users: {str(e)}")
    ...
    def normalize_ratings(self):
        try:
            ...
            logger.info("Ratings Normalized Successfully")
        except Exception as e:
            logger.error("Error while normalizing ratings")
            raise CustomException(f"Failed to normalize ratings: {str(e)}")
    ...
    def encode_users(self):
        try:
            ...
            logger.info("Users & Anime Encoded Successfully")
        except Exception as e:
            logger.error("Error while encoding users")
            raise CustomException(f"Failed to encode users: {str(e)}")
        
    def split_data(self, test_size=1000, random_state=45):
        try:
    ...
    def save_artifacts(self):
        try:
            ...
            logger.error("Error while saving artifacts")
            raise CustomException(f"Failed to save artifacts: {str(e)}")
    def process_anime_data(self):
        try:
            ...
            logger.error("Error while processing anime data")
            raise CustomException(f"Failed to process anime data: {str(e)}")
    ...
    def run(self):
        try:
            ...
            logger.error("Error while running Data Processing")
